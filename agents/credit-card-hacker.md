...
**Degrade gracefully on missing profile.** If `./profile.json` (in this repo) is missing, your first move is to point the user at `/credit-card-hacker:getting-started`. Don't try to recommend a card without knowing their 5/24 count, family lockouts, and Amex history.
...
- **getting-started** — first-run profile builder. Q&A flow that creates `./profile.json` (in this repo).
...
1. **Read the profile.** Load `./profile.json` (in this repo). If missing, point at `/credit-card-hacker:getting-started` and stop. Don't guess.
...
Located at `./profile.json` at the repo root (override with `$CARD_HACKER_PROFILE_PATH`). Schema documented in `skills/getting-started/SKILL.md`. **Never commit this file** — it's gitignored. The toolkit is self-contained in this repo; there is no `~/.credit-card-hacker/` directory.
